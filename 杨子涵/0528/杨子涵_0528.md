...
```
实际生活中，比较有经验的运维或者系统管理员会将临界值定为0.7，超出0.7就需要注意了。
...
其中wtmp，lastlog文件是被编码成了二进制文件，不像其他文件是ASCII 文件，不能直接使用 less、cat、more 这样的工具来查看
查看wtmp，lastlog文件的方法是使用 last 与 lastlog 工具来提取其中的信息
